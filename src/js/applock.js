...
(function () {
  ...
  function qaSkipAsk(items, i, afterAll) {
    textAsk({
      title: '跳过开屏问答', sub: '暗号是 99 + mochi 字卡生日的 4 位数字，一共 6 个数字。输入暗号后，这台设备以后每次打开都不再问答（不再显示问答层）。提示：时间就在开屏里可以找到，非常简单。警告：一旦有人把密码二传（告诉别人），发现后密码就会被重新设置，请不要外传。不输入暗号也不影响正常使用。',
