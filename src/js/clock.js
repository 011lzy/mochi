...
(function () {
  ...
  function setupCardLockCard() {
    ...
    if (tip) tip.textContent = open
      ...
      : '系统内置字卡已全部锁定，这是面向未成年人的保护措施，不是 bug。不输密码也能正常使用全部功能，密码只管两件事：解锁系统内置字卡、跳过开屏的 2 个问答。注意：锁定时若自定义字卡（含 mj 字卡）一张都没添加，回复会只能重复发兜底内容（如「嗯嗯」），自己在自定义字卡里添加几张即可。密码非常简单，答案就在开屏里可以找到；解开密码请勿二传（不要告诉别人），一旦有人二传，密码就会被重新设置。';
    actions.innerHTML = '';
